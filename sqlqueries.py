...
'''Get Requests'''
get_all_rides = "SELECT ActivityId, ActivityName FROM Ride NATURAL JOIN Athlete NATURAL JOIN Auth WHERE ActivityName LIKE '%{query}%' AND Username='{username}';"
get_ride = "SELECT ActivityId, ActivityName FROM Ride NATURAL JOIN Athlete NATURAL JOIN Auth WHERE ActivityId = {activity_id} AND Username='{username}';"
...
'''Post Requests'''
add_ride = "INSERT INTO Ride (AthleteID, ActivityName, Time) VALUES ((SELECT AthleteID FROM Auth WHERE Username='{username}'), '{activity_name}', '{time}');"
...
register_user = "INSERT INTO Auth (Username, Salt, Hash, AthleteID) VALUES ('{username}', '{salt}', '{hashed_password}', {athlete_id})"
...
add_athlete = "INSERT INTO Athlete VALUES()"
